...
namespace WebShoppen
{
            ...
            var welcomeText = new List<string>
    {
        ...
        "Find the best products at great prices!"
            ...
            Helper.PressKeyToContiniue();
        }

        public static async Task ShowShopPage( User user)
        {
            Console.Clear();
            using var db = new AppDbContext();
            ...
            Console.Write("Enter category ID to browse: ");
            ...
            await ShowProductDetails( user, productId);
        }

        public static async Task ShowProductDetails( User user, int productId)
        {
            using var db = new AppDbContext();
            ...
            if (quantity > 0)
            {
                await ProductService.AddToCart( user, productId, quantity);
        ...
        public static void ShowCartPage( User user)
        {
            ...
            Console.Write("Enter product ID to remove (0 to proceed): ");
            int productId = Helper.GetValidInteger();
            if (productId > 0)
            {
